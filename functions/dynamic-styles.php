...
/* ------------------------------------ */
if ( ! function_exists( 'cardstyle_hex2rgb' ) ) {

	function cardstyle_hex2rgb( $hex, $array=false ) {
		$hex = str_replace("#", "", $hex);
...
/*  Google fonts
/* ------------------------------------ */
if ( ! function_exists( 'cardstyle_enqueue_google_fonts' ) ) {

	function cardstyle_enqueue_google_fonts () {
...
add_action( 'wp_enqueue_scripts', 'cardstyle_enqueue_google_fonts' ); 	


/*  Dynamic css output
/* ------------------------------------ */
if ( ! function_exists( 'cardstyle_dynamic_css' ) ) {

	function cardstyle_dynamic_css() {
		if ( get_theme_mod('dynamic-styles', 'on') == 'on' ) {
		
			// rgb values
			$color_1 = get_theme_mod('color-1');
			$color_1_rgb = cardstyle_hex2rgb($color_1);
...
.entry a { color: '.esc_attr( get_theme_mod('color-1') ).'; }
			...
			// gradient left
			if ( get_theme_mod('gradient-left','#98ca2f') != '#98ca2f' ) {
				$styles .= '
#wrapper-bg,
#profile-image,
.blog-card-more-link,
...
.alx-tabs-nav li.active a { background: linear-gradient(90deg, '.esc_attr( get_theme_mod('gradient-left') ).' 0%, '.esc_attr( get_theme_mod('gradient-right') ).' 100%); }
			...
			// gradient right
			if ( get_theme_mod('gradient-right','#1a9b41') != '#1a9b41' ) {
				$styles .= '
#wrapper-bg,
#profile-image,
.blog-card-more-link,
...
.alx-tabs-nav li.active a { background: linear-gradient(90deg, '.esc_attr( get_theme_mod('gradient-left') ).' 0%, '.esc_attr( get_theme_mod('gradient-right') ).' 100%); }
			...
			if ( get_theme_mod('dark','off') == 'on' ) { 
				wp_add_inline_style( 'cardstyle-dark', $styles );
			} else {
				wp_add_inline_style( 'cardstyle-style', $styles );
			}
		}
	}
	
}
add_action( 'wp_enqueue_scripts', 'cardstyle_dynamic_css' );
